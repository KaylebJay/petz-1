...
petz.poop = function(self, pos)
	if not(petz.settings.poop) or not(self.tamed) or not(self.poop_rate) or self.child == true or self.object:get_velocity().y ~= 0 or math.random(1, petz.settings.poop_rate) > 1 then
		return
	end
